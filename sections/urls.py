from django.conf.urls import url
from . import views

urlpatterns = [
    url(r'^semesters/$',
        views.SemesterListView.as_view(),
        name='semester_list'),
    url(r'^semesters/new/$', views.SemesterAddView.as_view(),
        name='new_semester'),
    url(r'^semesters/(?P<pk>\d+)/edit/$',
        views.SemesterUpdateView.as_view(),
        name='semester_edit'),
    url(r'^semesters/(?P<pk>\d+)/delete/$',
        views.SemesterDeleteView.as_view(),
        name='semester_delete'),
    url(r'^helpers/$',
        views.HelperListView.as_view(),
        name='helper_list'),
    url(r'^sections/$',
        views.SectionListView.as_view(),
        name='section_list'),
    url(r'^sections/new/$', views.SectionAddView.as_view(),
        name='new_section'),
    url(r'^sections/(?P<pk>\d+)/edit/$',
        views.SectionUpdateView.as_view(),
        name='section_edit'),
    url(r'^sections/(?P<pk>\d+)/delete/$',
        views.SectionDeleteView.as_view(),
        name='section_delete'),
    url(r'^section/open/(?P<section>\d+)/$',
        views.SectionOpenView.as_view(),
        name='section_open'),
    url(r'^section/close/(?P<section>\d+)/$',
        views.SectionCloseView.as_view(),
        name='section_close'),
    url(r'^sections/load/$',
        views.SectionLoadView.as_view(),
        name='load_sections'),

    url(r'^presections/$',
        views.PreSectionListView.as_view(),
        name='presection_list'),
    url(r'^presections/new/$', views.PreSectionAddView.as_view(),
        name='new_presection'),
    url(r'^presections/(?P<pk>\d+)/edit/$',
        views.PreSectionUpdateView.as_view(),
        name='presection_edit'),
    url(r'^presections/(?P<pk>\d+)/delete/$',
        views.PreSectionDeleteView.as_view(),
        name='presection_delete'),
    url(r'^presections/(?P<presection>\d+)/refresh/$',
        views.PreSectionRefreshView.as_view(),
        name='presection_refresh'),
    url(r'^presections/load/$',
        views.PreSectionLoadView.as_view(),
        name='load_presections'),
    ]
