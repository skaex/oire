...
class ReportDetailView(View, LoginRequiredMixin):
    def get(self, request, *args, **kwargs):
        ...
        if self.request.user.id not in section.instructors.values_list('id', flat=True) and 'Special' not in groups:
            section_dean = '%s Dean' %(section.course.school.school)
            if section_dean not in groups:
